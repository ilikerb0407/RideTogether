...
struct Provider: TimelineProvider {
    static let emptyLocationSet = [TPBikeModel]()
    ...
    // MARK: Helpers
    private func contentUpdate(context: TimelineProvider.Context, locations: [TPBikeModel], updatedUserLocation: CLLocation, completionHandler: @escaping (SimpleEntry) -> Void) {
    ...
    private func loadNearestLocations(userLocation: CLLocation,
                                      completion: @escaping ([TPBikeModel]) -> Void) {
        ...
        URLSession.shared.dataTask(with: url, completionHandler: { data, _, error in
            ...
            do {
                let bikeData = try decoder.decode([TPBikeModel].self, from: data)
    ...
    private func closestLocations(userLocation: CLLocation, stationLocations: [TPBikeModel]) -> [TPBikeModel] {
        var allDistancesToUser = [(Double, TPBikeModel)]()
        ...
        let closestStationsToUser = closestStationDistancesToUser.map { _, station -> TPBikeModel in

            let updatedStationWithDistance = TPBikeModel(sno: station.sno,
...
struct SimpleEntry: TimelineEntry {
    let date: Date
    let nearestStations: [TPBikeModel]

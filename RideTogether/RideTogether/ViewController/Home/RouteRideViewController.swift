...
class RouteRideViewController: BaseViewController, CLLocationManagerDelegate, StopWatchDelegate {
    ...
    @IBOutlet weak var routeMap: GPXMapView!
...
//       if let inputUrl = URL(string: inputUrlString)
    ...
    func didLoadGPXFile(gpxRoot: GPXRoot) {
        ...
        routeMap.importFromGPXRoot(gpxRoot)
        
        routeMap.regionToGPXExtent()
    }
    ...
    func updatePolylineColor() {
        ...
        for overlay in routeMap.overlays where overlay is MKPolyline {
            
            routeMap.removeOverlay(overlay)
            
            routeMap.addOverlay(overlay)
        }
    }
    ...
    private var gpxTrackingStatus: GpxTrackingStatus = GpxTrackingStatus.notStarted {
        ...
        didSet {
            ...
            case .notStarted:
                ...
                totalTrackedDistanceLabel.distance = (routeMap.session.totalTrackedDistance)
                
                currentSegmentDistanceLabel.distance = (routeMap.session.currentSegmentDistance)
            ...
            case .paused:
                ...
                self.routeMap.startNewTrackSegment()
            }
        }
    }
    ...
    private var followUser: Bool = true {
        ...
        didSet {
            ...
            if followUser {
                ...
                routeMap.setCenter((routeMap.userLocation.coordinate), animated: true)
    ...
    @objc func addPinAtMyLocation() {
        ...
        let waypoint = GPXWaypoint(coordinate: locationManager.location?.coordinate ?? routeMap.userLocation.coordinate, altitude: altitude)
        routeMap.addWaypoint(waypoint)
        routeMap.coreDataHelper.add(toCoreData: waypoint)
    ...
    @objc func addPinAtTappedLocation(_ gesture: UILongPressGestureRecognizer) {
        ...
        if gesture.state == UIGestureRecognizer.State.began {
            ...
            let point: CGPoint = gesture.location(in: self.routeMap)
            routeMap.addWaypointAtViewPoint(point)
    ...
    func setUpMap() {
        ...
        routeMap.delegate = mapViewDelegate
        ...
        routeMap.addGestureRecognizer(panGesture)
        
        routeMap.rotationGesture.delegate = self
        ...
        routeMap.addGestureRecognizer(UILongPressGestureRecognizer( target: self,
                                                               action: #selector(JourneyViewController.addPinAtTappedLocation(_:))))
        
        self.view.addSubview(routeMap)
    ...
    @objc func saveButtonTapped(withReset: Bool = false) {
        ...
        let saveAction = UIAlertAction(title: "儲存",
            ...
            let gpxString = self.routeMap.exportToGPXString()

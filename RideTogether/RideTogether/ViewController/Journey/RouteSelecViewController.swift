...
class RouteSelecViewController: BaseViewController {
    ...
    @IBOutlet var showtempLabel: UILabel!
    ...
    func weather() {
        ...
        func showWeatherInfo() {
            guard let feelslike = weatherdata?.main.feelsLike.roundDouble() else {
                return
            }
            feelsLikeTempLabel.text = "\(feelslike) °C"
            guard let humiditydata = weatherdata?.main.humidity else {
                return
            }
            humidityLabel.text = "\(humiditydata) %"

            guard let tempdata = weatherdata?.main.tempMax.roundDouble() else {
                return
            }
            showtempLabel.text = "\(tempdata) °C"

            guard let ssunrise = weatherdata?.sys.sunrise else {
                return
            }
            ...
            guard let ssunset = weatherdata?.sys.sunset else {
                return
            }
            ...
            guard let swind = weatherdata?.wind.speed.roundDouble() else {
                return
            }
            windLabel.text = "\(swind) km/h"

            guard let weather = weatherdata?.weather[0].main else {
                return
            }
    ...
    @objc
    private func suggestionTapped(_: UITapGestureRecognizer) {
        hideSuggestionView(animated: true)
...
//                self.present(viewController, animated: true)
            ...
            case let .failure(error):
                ...
                switch error {
                case let .invalidSegment(reason):
